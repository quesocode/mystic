...
NSString *const kMysticTwoInputTextureVertexShaderString = SHADER_STRING
(
...
@implementation MysticTwoInputFilter
...
{
    if (!(self = [self initWithVertexShaderFromString:kMysticTwoInputTextureVertexShaderString fragmentShaderFromString:fragmentShaderString]))
    {
		return nil;
    }
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    if ((hasReceivedFirstFrame && hasReceivedSecondFrame) || updatedMovieFrameOppositeStillImage)
    {
        ...
        [super newFrameReadyAtTime:passOnFrameTime atIndex:1]; // Bugfix when trying to record: always use time from first input (unless indefinite, in which case use the second input)
        hasReceivedFirstFrame = NO;
        hasReceivedSecondFrame = NO;
    }
}
