...
- (UIBezierPath *) inverseInRect:(CGRect)rect bounds:(CGRect)pathBounds;
{
    UIBezierPath *path = [UIBezierPath bezierPath];
    UIBezierPath *selfPath = [[self copy] autorelease];
    FitPathToRect(selfPath, rect);
    selfPath = MovePathToPoint(selfPath, rect.origin);
    CopyBezierState(path, selfPath);
    [path appendPath:selfPath];
    [path appendPath:[UIBezierPath bezierPathWithRect:pathBounds]];
    path.usesEvenOddFillRule = YES;
    return path;
}
