...
UIBezierPath * MovePathToPoint(UIBezierPath *path, CGPoint destPoint)
{
    UIBezierPath *newPath = path.copy;
    CGRect bounds = PathBoundingBox(newPath);
    CGPoint p1 = bounds.origin;
    CGPoint p2 = destPoint;
    CGSize vector = CGSizeMake(p2.x - p1.x, p2.y - p1.y);
    OffsetPath(newPath, vector);
...
UIBezierPath * FlipPathHorizontally(UIBezierPath *path)
{
    MirrorPathHorizontally(path);
    return path;

}
UIBezierPath * FlipPathVertically(UIBezierPath *path)
{
    MirrorPathVertically(path);
    return path;

}
UIBezierPath * FlipPath(UIBezierPath *path)
{
    if(!path) return nil;
...
UIBezierPath * SetPathRect(UIBezierPath *path, CGRect destRect)
{
    if(!path) return nil;
    ...
    CGPoint p1 = bounds.origin;
    CGPoint p2 = destRect.origin;
    CGSize p3 = CGSizeMake(p2.x - p1.x, p2.y - p1.y);
    
    ScalePath(p, scale, scale);
    
    CGRect b2 = PathBoundingBox(p);
    CGPoint p4 = b2.origin;
    CGSize p5 = CGSizeMake(p2.x - p4.x, p2.y - p4.y);

    OffsetPath(p, p5);
    CGRect b3 = PathBoundingBox(p);

    return [p autorelease];
}
UIBezierPath * FitPathToRect(UIBezierPath *path, CGRect destRect)
{
    if(!path) return nil;
    ...
    UIBezierPath *p = [path copy];
    MovePathCenterToPoint(p, newCenter);
    ScalePath(p, scale, scale);
    return [p autorelease];
}
...
UIBezierPath *BezierPolygon(NSUInteger numberOfSides, CGRect destinationRect)
{
    if (numberOfSides < 3)
    {
    ...
    destinationRect = CGRectUnknownOrZero(destinationRect) ? CGRectMake(0, 0, 1, 1) : destinationRect;

    UIBezierPath *path = [UIBezierPath bezierPath];
    CGPoint center = RectGetCenter(destinationRect);
    CGFloat m = MIN(destinationRect.size.width, destinationRect.size.height);
    CGFloat r = m/2;
...
UIBezierPath *BezierInflectedShape(NSUInteger numberOfInflections, CGFloat percentInflection, CGRect destinationRect)
{
    ...
    CGPoint center = RectGetCenter(destinationRect);
    CGFloat m = MIN(destinationRect.size.width, destinationRect.size.height);
    CGFloat r = (m/2)/(1.0+percentInflection);
...
UIBezierPath *BezierStarShape(NSUInteger numberOfInflections, CGFloat percentInflection, CGRect destinationRect)
{
    if (numberOfInflections < 3)
    {
        NSLog(@"Error: Please supply at least 3 inflections");
...
void EmbossPath(UIBezierPath *path, UIColor *color, UIColor *darkColor, CGSize radius, CGSize radiusDark, CGFloat blur)
{
    if (!path) COMPLAIN_AND_BAIL(@"Path cannot be nil", nil);
    CGContextRef context = UIGraphicsGetCurrentContext();
    if (context == NULL) COMPLAIN_AND_BAIL(@"No context to draw into", nil);
    ...
    UIColor *contrast = darkColor ? darkColor : ContrastColor(color);
    if(radius.width <= 0)
    {
        DrawInnerShadow(path, color, CGSizeMake(radius.width, radius.height), blur);
        DrawInnerShadow(path, contrast, CGSizeMake(radiusDark.width, radiusDark.height), blur);
...
//
//void EmbossPath(UIBezierPath *path, UIColor *color, UIColor *darkColor, CGSize radius, CGFloat blur)
//{
//    if (!path) COMPLAIN_AND_BAIL(@"Path cannot be nil", nil);
//    CGContextRef context = UIGraphicsGetCurrentContext();
//    if (context == NULL) COMPLAIN_AND_BAIL(@"No context to draw into", nil);
...
// Half an emboss
void InnerBevel(UIBezierPath *path,  UIColor *color, UIColor *shadowColor, CGFloat radius, CGFloat blur, CGFloat theta)
{
    if (!path) COMPLAIN_AND_BAIL(@"Path cannot be nil", nil);
    if (!color) COMPLAIN_AND_BAIL(@"Color cannot be nil", nil);
    CGContextRef context = UIGraphicsGetCurrentContext();
    ...
    shadowColor = shadowColor ? shadowColor : [color colorWithAlphaComponent:0.5f];
    DrawInnerShadow(path, shadowColor, CGSizeMake(-x, y), blur);
}
...
// Typically call with black color at 0.5
void BevelPath(UIBezierPath *path,  UIColor *color, UIColor *shadowColor,  CGFloat radius, CGFloat blur, CGFloat theta)
{
    if (!path) COMPLAIN_AND_BAIL(@"Path cannot be nil", nil);
    if (!color) COMPLAIN_AND_BAIL(@"Color cannot be nil", nil);
    CGContextRef context = UIGraphicsGetCurrentContext();
    ...
    DrawInnerShadow(path, shadowColor ? shadowColor : [color darker:0.5], CGSizeMake(-x, y), blur);
...
{
    [self clipToStroke:width miterLimit:4];
}
- (void) clipToStroke:(NSUInteger)width miterLimit:(float)miterLimit;
{
    CGPathRef pathRef = CGPathCreateCopyByStrokingPath(self.CGPath, NULL, width, kCGLineCapButt, kCGLineJoinMiter, miterLimit);
    UIBezierPath *clipPath = [UIBezierPath bezierPathWithCGPath:pathRef];
    CGPathRelease(pathRef);
    [clipPath addClip];
}
- (UIBezierPath *) pathWithStroke:(NSUInteger)width;
{
    return [self pathWithStroke:width miterLimit:4];
}

- (UIBezierPath *) pathWithStroke:(NSUInteger)width miterLimit:(float)miterLimit;
{
    CGPathRef pathRef = CGPathCreateCopyByStrokingPath(self.CGPath, NULL, width, kCGLineCapButt, kCGLineJoinMiter, miterLimit);
    UIBezierPath *clipPath = [UIBezierPath bezierPathWithCGPath:pathRef];
    CGPathRelease(pathRef);
...
- (void) clipToInverseStroke:(NSUInteger)width
{
    [self clipToInverseStroke:width miterLimit:4];
}
- (void) clipToInverseStroke:(NSUInteger)width miterLimit:(float)miterLimit;
{
    CGPathRef pathRef = CGPathCreateCopyByStrokingPath(self.CGPath, NULL, width, kCGLineCapButt, kCGLineJoinMiter, miterLimit);
    UIBezierPath *clipPath = [UIBezierPath bezierPathWithCGPath:pathRef];
    CGPathRelease(pathRef);
    [clipPath.inverse addClip];
}
