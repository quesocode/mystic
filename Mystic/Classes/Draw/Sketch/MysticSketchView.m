...
#import "MysticSketchView.h"
#import "UIColor+Mystic.h"

@implementation MysticSketchView
{
...
- (void) initCommon
{
    _setting = MysticSketchToolTypeMask;
    self.backgroundColor = [UIColor whiteColor];
    _toolType = MysticSketchToolTypePaint;
...
- (void) setDrawColor:(UIColor *)drawColor;
{
    ...
    switch (self.setting) {
        case MysticSketchToolTypeMaskErase:
            self.color2 = [UIColor clearColor];
            break;
...
- (void) setAirBrushFlow:(CGFloat)airBrushFlow
{
    ...
    if(_airBrushFlow > 0 && self.toolType == MysticSketchToolTypePaint)
    {
        self.toolType = MysticSketchToolTypeAirBrush;
    }
    else if(_airBrushFlow == 0 && self.toolType != MysticSketchToolTypePaint)
    {
        self.toolType = MysticSketchToolTypePaint;
    }
}
...
//    NSInteger num = CGColorGetNumberOfComponents(self.drawColor.CGColor);
//    CGFloat fc = sinf(((self.airBrushFlow/5.0f)*M_PI)/2.0f);
//    fc = (1-self.airBrushFlow) + self.airBrushFlow/2;
//    DLog(@"drawColor:  %@   %2.1f    %d :  %2.1f,  %2.1f,  %2.1f", ColorToString(self.drawColor), fc, (int)num, comp[0],comp[1],comp[2]);
    
//    CGFloat colors[8] = { comp[0], comp[1], comp[2], 0.0f, comp[0], comp[1], comp[2], fc };
//    CGFloat r,g,b;
//    r = self.drawColor.red;
//    g = self.drawColor.green;
//    b = self.drawColor.blue;
    
    
    CGContextRef context = UIGraphicsGetCurrentContext();
    
    //// Gradient Declarations
//    CGFloat gradientLocations[] = {1-self.airBrushFlow, (1-self.airBrushFlow) + self.airBrushFlow/2, 1};
    CGFloat gradientLocations[] = {0, (1-self.airBrushFlow) + self.airBrushFlow/2, 1};

    DLog(@"locations: %2.1f, %2.1f, %2.1f", gradientLocations[0], gradientLocations[1], gradientLocations[2]);
    CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)@[(id)self.drawColor.CGColor, (id)[self.drawColor blendWithColor:self.color2 alpha:0.5].CGColor, (id)self.color2.CGColor], gradientLocations);
    
//    CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)@[(id)self.drawColor.CGColor, (id)[UIColor redColor].CGColor, (id)[UIColor blueColor].CGColor], gradientLocations);

    
    //// Oval Drawing
    CGRect ovalRect = CGRectMake(0, 0, self.drawWidth, self.drawWidth);
...
//    
//    CGFloat colors[8] = { r, g, b, 0.0f, r, g, b, fc };
//
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.toolType == MysticSketchToolTypePaint) {
        [self paintTouchesBegan];
    }
    if (self.toolType == MysticSketchToolTypeAirBrush) {
