...
-(void) setAccesoryViewsHidden: (BOOL) hidden
{
    ...
    if(self.rightSwipeSettings.allowSwipeDuringEditing && self.editingAccessoryView)
    {
        self.editingAccessoryView.hidden = hidden;
    }
...
- (void) setDelegate:(id<MGSwipeTableCellDelegate>)delegate;
{
    _delegate = delegate;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (expansion) {
            ...
            __unsafe_unretained __block MGSwipeButtonsView * _expansion = expansion;
            __unsafe_unretained __block UIView * _expandedButton = expandedButton;
            __unsafe_unretained __block UIColor *_bgColor = backgroundColor;
            [self setSwipeOffset:_targetOffset animation:expSettings.triggerAnimation completion:^(BOOL finished){
                if (!finished || self.hidden)  return; //cell might be hidden after a delete row animation without being deallocated (to be reused later)
                if ([_expansion handleClick:_expandedButton fromExpansion:YES]) [_expansion endExpansionAnimated:NO];
                if (_bgColor) _expandedButton.backgroundColor = _bgColor;
            }];
        }
        else {
