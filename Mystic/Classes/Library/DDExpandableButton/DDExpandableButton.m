...
- (void)dealloc
{
    if(_action) [self removeTarget:self action:@selector(actionTapped:) forControlEvents:UIControlEventValueChanged];
...
- (void) setAction:(MysticBlockButtonExpandable)action;
{
    BOOL was = _action != nil;
    if(_action) {
        Block_release(_action);
        _action = nil;
    }
    ...
    if(!was && _action)
    {
        [self addTarget:self action:@selector(actionTapped:) forControlEvents:UIControlEventValueChanged];
    }
