...
-(void)commonInit
{
    ...
    UIColor *bgColor = [[UIColor hex:@"151515"] colorWithAlphaComponent:0.4f];
    ...
    self.leftOverlayView.backgroundColor = bgColor;
    [self addSubview:self.leftOverlayView];
    
    self.rightOverlayView = [[UIView alloc] init];
    self.rightOverlayView.backgroundColor = bgColor;
...
- (void) setFlipHorizontal:(BOOL)flipHorizontal;
{
    ...
    self.imageView.transform = CGAffineTransformScale(self.imageView.transform, flipHorizontal?-1.0:-1.0, 1.0);
}
- (void) setFlipVertical:(BOOL)flipVertical;
{
    _flipVertical=flipVertical;
    self.imageView.transform = CGAffineTransformScale(self.imageView.transform, 1.0, flipVertical?-1.0:-1.0);
}
#pragma mark -
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(self.zoomingView.frame, zoomedPoint)) return self.scrollView;
...
- (void)layoutSubviews
{
    ...
    if (!self.imageView) {
        if (UIInterfaceOrientationIsPortrait(interfaceOrientation)) self.insetRect = CGRectInset(self.bounds, MarginLeft, MarginTop);
        else  self.insetRect = CGRectInset(self.bounds, MarginLeft, MarginLeft);
        [self setupImageView];
    }
    
    if (!self.isResizing) {
        [self layoutCropRectViewWithCropRect:self.scrollView.frame];
        if (self.interfaceOrientation != interfaceOrientation) [self zoomToCropRect:self.scrollView.frame];
    }
...
{
    if(!CGSizeIsZero(self.returnSize)) self.returnSize = CGSizeZero;
    [self zoomToCropRect:cropRect];
}
...
{
    if(!CGSizeIsZero(self.returnSize))
    {
        return [self.image rotatedImageWithtransform:self.rotation croppedToRect:self.zoomedCropRect size:self.returnSize];
    }
    UIImage *c = [self.image rotatedImageWithtransform:self.rotation croppedToRect:self.zoomedCropRect];
    return c;
}
...
{
    [self zoomToCropRect:toRect force:NO animated:YES];
}
- (void)zoomToCropRect:(CGRect)toRect force:(BOOL)force animated:(BOOL)animated;
{
    if (!force && CGRectEqualToRect(self.scrollView.frame, toRect)) {
        return;
    }
...
//        if(self.flipVertical) self.flipVertical = YES;
//        if(self.flipHorizontal) self.flipHorizontal = YES;
