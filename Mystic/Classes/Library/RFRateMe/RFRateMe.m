...
#define kNumberOfDaysUntilShowAgain MYSTIC_RATE_AGAIN_AFTER_DAYS
#define kAppStoreAddress MYSTIC_WEB_ADDRESS
#define kAppName MYSTIC_NAME

@implementation RFRateMe

+(void)showRateAlert {
    [self showRateAlert:NO];
}
+(void)showRateAlert:(BOOL)force {
    ...
    if (remindMeLater) {
        ...
        if ((long)[components day] <= kNumberOfDaysUntilShowAgain && !force) return;
