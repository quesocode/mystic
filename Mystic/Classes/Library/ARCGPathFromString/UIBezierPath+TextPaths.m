...
+ (UIBezierPath *)pathForString:(NSString *)string withFont:(UIFont *)font
{
    ...
    if (!string || !font || string.length == 0 || [string removeWhitespaces].length == 0) return nil;
...

{
    ...
    if (!string || !font ||  maxWidth <= 0.0 || string.length == 0 || [string removeWhitespaces].length == 0) return nil;
...
+ (UIBezierPath *)pathForAttributedString:(NSAttributedString *)string
{
    ...
    if (!string || string.length == 0 || [string.string removeWhitespaces].length == 0) return nil;
...
+ (UIBezierPath *)pathForMultilineAttributedString:(NSAttributedString *)string maxWidth:(CGFloat)maxWidth
{
    ...
    if (!string || maxWidth <= 0.0 || string.length == 0 || [string.string removeWhitespaces].length == 0) return nil;
    
    // create the path from the specified string.
    CGPathRef letters = CGPathCreateMultilineStringWithAttributedString(string, maxWidth, MAX_HEIGHT_OF_FRAME);
    if(!letters) return nil;
    // make an iOS UIBezierPath object from the CGPath.
    UIBezierPath *path = [UIBezierPath bezierPathWithCGPath:letters];
...
+ (UIBezierPath *)pathForAttributedString:(NSAttributedString *)string maxWidth:(CGFloat)maxWidth lineCount:(int)numnberOfLines;
{
    if (!string || maxWidth <= 0.0 || string.length == 0 || [string.string removeWhitespaces].length == 0) return nil;

    if(numnberOfLines > 1)
    {
        if(maxWidth <= 0) maxWidth = string.size.width;
        return [UIBezierPath pathForMultilineAttributedString:string maxWidth:maxWidth];
    }
    return [UIBezierPath pathForAttributedString:string];
}
