...
public typealias blockCompletionCapturePhoto = (_ image: UIImage?, _ error: NSError?) -> (Void)
...
class CameraEngineCaptureOutput: NSObject {
    ...
    fileprivate let stillCameraOutput = AVCaptureStillImageOutput()
...
//        self.stillCameraOutput.captureStillImageAsynchronously(from: connectionVideo) { (sampleBuffer: CMSampleBuffer!, err: NSError!) -> Void in
            if let err = err {
...
extension CameraEngineCaptureOutput: AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    fileprivate func progressCurrentBuffer(_ sampleBuffer: CMSampleBuffer) {
