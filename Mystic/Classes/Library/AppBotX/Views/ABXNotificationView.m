...
{
    return [ABXNotificationView show:text actionText:actionText closeText:@"Close" backgroundColor:backgroundColor textColor:textColor buttonColor:buttonColor inController:controller actionBlock:actionBlock dismissBlock:dismissBlock];
}
+ (ABXNotificationView*)show:(NSString*)text
                  actionText:(NSString*)actionText
                  closeText:(NSString*)closeText
             backgroundColor:(UIColor*)backgroundColor
                   textColor:(UIColor*)textColor
                 buttonColor:(UIColor*)buttonColor
                inController:(UIViewController*)controller
                ...
                dismissBlock:(ABXNotificationViewCallback)dismissBlock
{
    ...
    UIFont *font = [MysticFont font:14];
    ...
    // Create the view
    CGFloat totalHeight = labelHeight + 70 + topPadding;
    ...
    // Label for the text
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(view.bounds) - kMaxWidth)/2, 20 + topPadding, kMaxWidth, labelHeight)];
    ...
    if (actionText && actionText.length > 0) {
        ...
        button.titleLabel.font = [MysticFont fontBold:15];
        [button addTarget:view action:@selector(onAction:) forControlEvents:UIControlEventTouchUpInside];
        [view addSubview:button];
    }
    ALLog(@"close button", @[@"action", MObj(actionText),
                             @"act len", actionText ? @(actionText.length) : @(-1),
                             @"closeText", MObj(closeText),
                             @"local", MObj([closeText localizedString]),
                             ]);
    // Close Button
    UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
    button.tintColor = buttonColor;
    [button setTitle:[closeText localizedString] forState:UIControlStateNormal];
    if (actionText && actionText.length > 0) {
        button.frame = CGRectMake(CGRectGetWidth(view.bounds) / 2, totalHeight - 45, kMaxWidth / 2, 35);
    }
    else {
        button.frame = CGRectMake(0, totalHeight - 45, CGRectGetWidth(view.bounds), 35);
    }
    button.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    button.titleLabel.font = [MysticFont fontBold:12];
